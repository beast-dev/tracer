...
If multiple trace files contain the same collection of parameters,  then a ``Combined'' trace appears automatically. %GB: or have the same collection of logged parameters
...
%
\begin{figure}[t]
\subfigure[Tracer overview]{\includegraphics[width=0.485\textwidth]{./figures/rabv-overview-3.pdf}} \\
...
These alternative packages compute, for example, Gelman-Rubin diagnostics \citep{gelman1992inference} that \tracer\ currently does not provide.
